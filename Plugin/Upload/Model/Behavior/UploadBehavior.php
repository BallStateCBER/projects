 ...
 **/
	...
	public function _getMimeType($filePath) {
		...
		/* Graham's edit: Fall back on MIME_Type PEAR utility class
		 * if mime_content_type() is not supported on this server
		 */
		$mime_paths = array(
			'MIME/Type.php',
			'/MIME/Type.php',
			'/home/okbvtfr/php/MIME/Type.php'
		); 
		foreach ($mime_paths as $path) {
			if (is_file($path)) {
				require_once $path;
