<?php
/**
 * Routes configuration
 *
 * In this file, you set up routes to your controllers and their actions.
 * Routes are very important mechanism that allows you to freely connect
 * different urls to chosen controllers and their actions (functions).
 *
 * PHP 5
 *
 * CakePHP(tm) : Rapid Development Framework (http://cakephp.org)
 * Copyright 2005-2012, Cake Software Foundation, Inc. (http://cakefoundation.org)
 *
 * Licensed under The MIT License
 * Redistributions of files must retain the above copyright notice.
 *
 * @copyright     Copyright 2005-2012, Cake Software Foundation, Inc. (http://cakefoundation.org)
 * @link          http://cakephp.org CakePHP(tm) Project
 * @package       app.Config
 * @since         CakePHP(tm) v 0.2.9
 * @license       MIT License (http://www.opensource.org/licenses/mit-license.php)
 */
/**
 * Here, we are connecting '/' (base path) to controller called 'Pages',
 * its action called 'display', and we pass a param to select the view file
 * to use (in this case, /app/View/Pages/home.ctp)...
 */

Router::connect('/', array('controller' => 'releases', 'action' => 'index'));
Router::connect('/search', array('controller' => 'releases', 'action' => 'search'));
Router::connect('/login', array('controller' => 'users', 'action' => 'login'));
Router::connect('/logout', array('controller' => 'users', 'action' => 'logout'));
Router::connect(
	'/:id/:slug', 
	array('controller' => 'releases', 'action' => 'view'),
	array('id' => '[0-9]+', 'slug' => '[-_a-z0-9]+', 'pass' => array('id', 'slug'))
);
Router::connect(
	'/partner/:id/:slug', 
	array('controller' => 'partners', 'action' => 'view'),
	array('id' => '[0-9]+', 'slug' => '[-_a-z0-9]+', 'pass' => array('id', 'slug'))
);
Router::connect(
	'/tag/:id/:slug', 
	array('controller' => 'tags', 'action' => 'view'),
	array('id' => '[0-9]+', 'slug' => '[-_a-z0-9]+', 'pass' => array('id', 'slug'))
);
Router::connect(
	'/year/:year', 
	array('controller' => 'releases', 'action' => 'year'),
	array('year' => '[0-9]+', 'pass' => array('year'))
);


CakePlugin::routes();
require CAKE . 'Config' . DS . 'routes.php';
